...
def load_inventory():
    try:
        ...
        # Realistic Default Inventory for a Restaurant
        df = pd.DataFrame({
...
def load_buffet_recipes():
    try:
        df = pd.read_csv("data/buffet_recipes.csv")
    except FileNotFoundError:
        # Default Buffet Menu
        df = pd.DataFrame({
        ...
        df.to_csv("data/buffet_recipes.csv", index=False)
    return df
...
# --- PAGE: TAKE ORDER ---
    ...
    if recipes_df.empty:
        ...
        if st.button("🛒 Process Order", type="primary", key="order_process_btn"):
            # Get ingredients for selected recipe
...
# --- PAGE: BUFFET ORDER ---
    ...
    if buffet_df.empty:
        ...
        if st.button("🍽️ Process Buffet Order", type="primary", key="buffet_process_btn"):
            # Get ingredients for selected buffet
...
# --- PAGE: EDIT DATA (FIXED DUPLICATE KEYS & UPDATED CATEGORIES) ---
    ...
    with tab1:
        ...
        if st.button("💾 Save Inventory Changes", type="primary", key="app_save_inventory"):
            edited_inventory.to_csv("data/inventory.csv", index=False)
            st.cache_data.clear()
        ...
        with st.expander("➕ Quick Add Item"):
            ...
            # ✅ UPDATED CATEGORIES TO MATCH app.py
            ...
            if st.button("➕ Add Item", key="app_inv_add"):
                ...
                updated_df.to_csv("data/inventory.csv", index=False)
                st.cache_data.clear()
    ...
    with tab2:
        ...
        with st.expander("➕ Add New Recipe"):
            ...
            if st.button("➕ Add Ingredient", key="app_recipe_add"):
                ...
                updated.to_csv("data/recipes.csv", index=False)
                st.cache_data.clear()
                st.success("Added!")
                st.rerun()
        ...
        if st.button("💾 Save Recipes", type="primary", key="app_save_recipes"):
            edited_recipes.to_csv("data/recipes.csv", index=False)
            st.cache_data.clear()
    ...
    with tab3:
        ...
        with st.expander("➕ Add New Buffet"):
            ...
            if st.button("➕ Add Ingredient to Buffet", key="app_buffet_add"):
                ...
                updated.to_csv("data/buffet_recipes.csv", index=False)
                st.cache_data.clear()
                st.success("Added!")
                st.rerun()
        ...
        if st.button("💾 Save Buffet Changes", type="primary", key="app_save_buffet"):
            edited_buffet.to_csv("data/buffet_recipes.csv", index=False)
            st.cache_data.clear()
