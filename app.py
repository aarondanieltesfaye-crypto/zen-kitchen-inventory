...
def _fix_legacy_categories(df):
    ...
    # 2. Force specific erroneous items to the correct category
    df.loc[df['item'] == 'Nile perch', 'category'] = 'Proteins'
    df.loc[df['item'] == 'Nile perch', 'category'] = 'Proteins'  # Handle lowercase/typos
    df.loc[df['item'] == 'Nile Perch', 'category'] = 'Proteins'
...
def load_inventory():
    ...
    if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
        try:
            ...
            df.to_csv(file_path, index=False) # Save the fix back to disk
            return df
        except pd.errors.EmptyDataError:
            # Realistic Default Inventory
            df = pd.DataFrame({
